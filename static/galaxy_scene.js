/* ============================================================
   KING DIADEM — galaxy_scene.js v10
   ★ Mobile-first responsive planets
   ★ scale() recalculates on resize
   ============================================================ */
(function () {
  ...
  /* ── PLANETS rebuilt on resize ── */
  var PLANETS = [];
  var _planetAngles = null;

  function rebuildPlanets(){
    var prev = PLANETS;
    PLANETS = [
      {color:'#b0b8cc', sz:sc(4),   orb:sc(62),  spd:.0042, ring:false, moon:false},
      {color:'#e8c87a', sz:sc(7),   orb:sc(98),  spd:.0032, ring:false, moon:false},
      {color:'#3a7bd5', sz:sc(8),   orb:sc(142), spd:.0022, ring:false, moon:true },
      {color:'#cc5533', sz:sc(6),   orb:sc(190), spd:.0018, ring:false, moon:false},
      {color:'#d4a060', sz:sc(17),  orb:sc(268), spd:.0009, ring:false, moon:false},
      {color:'#eedd98', sz:sc(14),  orb:sc(350), spd:.0006, ring:true,  moon:false},
      {color:'#88dde8', sz:sc(10),  orb:sc(425), spd:.0004, ring:false, moon:false},
      {color:'#2244cc', sz:sc(9),   orb:sc(490), spd:.0002, ring:false, moon:false},
      {color:'#60a5fa', sz:sc(5),   orb:sc(118), spd:.0016, ring:false, moon:false, kd:'WATERLINE'},
      {color:'#fff8e7', sz:sc(4.5), orb:sc(228), spd:.0013, ring:false, moon:false, kd:'VEGA'},
      {color:'#f87171', sz:sc(3.5), orb:sc(162), spd:.0028, ring:false, moon:false, kd:'HALT'},
      {color:'#a78bfa', sz:sc(4),   orb:sc(382), spd:.0004, ring:false, moon:false, kd:'CIVIL'},
    ];
    /* preserve angles across resize */
    var defaults = [0,1.2,2.5,4.0,1.8,3.2,5.5,2.8,0.5,3.5,1.0,4.5];
    PLANETS.forEach(function(p,i){
      p.ang = (prev[i] && prev[i].ang != null) ? prev[i].ang : defaults[i];
    });
  }
  ...
  function drawMW(){
    ...
    for(var i=0;i<MW_N;i++){
      ...
      var sc2=Math.max(.1,s.s*Math.min(1,420/d));
  ...
  /* ── PLANET RENDER — 7-layer depth ── */
  function hexRgb(hex){
    return [parseInt(hex.slice(1,3),16),parseInt(hex.slice(3,5),16),parseInt(hex.slice(5,7),16)];
  }

  function drawPlanet(x,y,p){
    var sz=p.sz, rgb=hexRgb(p.color), R=rgb[0],G=rgb[1],B=rgb[2];

    /* haze */
    var haze=ctx.createRadialGradient(x,y,sz*.6,x,y,sz*3.8);
    haze.addColorStop(0,'rgba('+R+','+G+','+B+',0.12)');
    haze.addColorStop(1,'rgba(0,0,0,0)');
    ctx.beginPath();ctx.arc(x,y,sz*3.8,0,Math.PI*2);ctx.fillStyle=haze;ctx.fill();
    ...
    ctx.beginPath();ctx.arc(x,y,sz*1.55,0,Math.PI*2);ctx.fillStyle=atm;ctx.fill();
    ...
    lit.addColorStop(0.65,'rgba('+Math.round(R*.55)+','+Math.round(G*.55)+','+Math.round(B*.55)+',0.70)');
    lit.addColorStop(1,'rgba(0,0,0,0)');
    ctx.beginPath();ctx.arc(x,y,sz,0,Math.PI*2);ctx.fillStyle=lit;ctx.fill();

    /* terminator shadow */
    var shadow=ctx.createRadialGradient(x-lx*sz*.35,y-ly*sz*.35,sz*.1,x-lx*sz*.35,y-ly*sz*.35,sz*1.1);
    shadow.addColorStop(0,'rgba(0,0,10,0)');
    shadow.addColorStop(.55,'rgba(0,0,10,0.30)');
    shadow.addColorStop(1,'rgba(0,0,10,0.75)');
    ctx.beginPath();ctx.arc(x,y,sz,0,Math.PI*2);ctx.fillStyle=shadow;ctx.fill();

    /* specular */
    var spec=ctx.createRadialGradient(x+lx*sz*.38,y+ly*sz*.38,0,x+lx*sz*.38,y+ly*sz*.38,sz*.32);
    ...
    /* rim */
    var rim=ctx.createRadialGradient(x-lx*sz*.7,y-ly*sz*.7,sz*.7,x-lx*sz*.7,y-ly*sz*.7,sz*1.1);
    rim.addColorStop(0,'rgba('+R+','+G+','+B+',0)');
    rim.addColorStop(1,'rgba('+Math.round(R*.5)+','+Math.round(G*.5)+','+Math.round(B*.5)+',0.20)');
    ctx.beginPath();ctx.arc(x,y,sz,0,Math.PI*2);ctx.fillStyle=rim;ctx.fill();
  }
  ...
  function drawPlanets(){
    PLANETS.forEach(function(p){
      ...
      if(p.ring){
        ...
        var rg=ctx.createRadialGradient(0,0,p.sz*1.9,0,0,p.sz*3.4);
        rg.addColorStop(0,'rgba(238,216,152,0)');
        rg.addColorStop(.2,'rgba(238,216,152,0.55)');
        rg.addColorStop(.7,'rgba(220,195,130,0.35)');
        rg.addColorStop(1,'rgba(200,170,100,0)');
      ...
      /* moon */
      if(p.moon){
        var ma=p.ang*8,mx=x+Math.cos(ma)*sc(18),my=y+Math.sin(ma)*sc(5);
        var msz=sc(2.2),mdx=CX-mx,mdy=CY-my,mdd=Math.hypot(mdx,mdy)||1;
        ...
        mg.addColorStop(.5,'rgba(160,170,185,0.75)');
        mg.addColorStop(1,'rgba(30,35,45,0.25)');
      ...
      if(p.kd){
        ...
        ctx.font='bold '+Math.max(7,Math.round(p.sz*.6))+'px "DM Mono",monospace';
